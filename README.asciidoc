...
Query parameters:
  ...
  unique-git-revision::
    If 1, do not register a new container if one already exists for that
    git revision

Response status codes:

  201::
    Container was successfully registered, see Location.

  302::
    A container already exists for this git-revision, see Location.

Response headers:

  Location::
    The base URL of the registered container.
