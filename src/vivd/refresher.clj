...
(defn- do-refresh-container [index id]
  (let [container (index/get index id)
        updated   (container/with-refreshed-status container)]
    (if (not (= container updated))
      (do
        (log/debug "Updating due to refresh" container)
        (index/update index updated)))))

(defn- do-refresh [index]
  (log/debug "Refreshing all...")
  (doall (map (partial do-refresh-container index) (index/keys index))))

(defn- refresh-loop [refresh-chan index]
  (go-loop []
...
(defn make [index]
  ...
  (let [refresh-chan (chan 50)]
    (refresh-loop refresh-chan index)
